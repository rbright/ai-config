...
This directory contains the Claude Code configuration (`claude/.config/claude/`) and the accompanying command/agent definitions. It’s kept intact so you can switch back to Claude Code in the future.
...
1. Manually copy the configuration from `.config/claude/` to `$HOME/.claude/`
...
```sh
# Install configuration (if not already installed)
stow claude
```
...
- **[Orchestrator](.config/claude/agents/orchestrator.md)** — Defines requirements and coordinates execution
...
### 🔨 Execution
- **[Backend Engineer](.config/claude/agents/backend-engineer.md)** — Server-side logic, APIs, data pipelines, and infrastructure
...
- **[Frontend Engineer](.config/claude/agents/frontend-engineer.md)** — User interfaces with React/TypeScript
  - Responsive, accessible UI components
  - State management and API integration

- **[Machine Learning Engineer](.config/claude/agents/ml-engineer.md)** — Model development and deployment
  - Training pipelines and experiments
  - Inference performance and deployment

### 🔍 Review
- **[Reviewer](.config/claude/agents/reviewer.md)** — QA, Code Review, and Security
...
1. **[Orchestrator](.config/claude/agents/orchestrator.md)** creates requirements.md
...
4. **[Orchestrator](.config/claude/agents/orchestrator.md)** synthesizes plans and identifies dependencies
...
3. **[Orchestrator](.config/claude/agents/orchestrator.md)** manages dependencies and parallel execution
4. Agents work independently on their task lists

### Review Process (`/user-review`)
Deploys the consolidated Reviewer:
- **[Reviewer](.config/claude/agents/reviewer.md)** validates functionality, code quality, and security
...
- **Clear Boundaries** - Each agent has exclusive authority over their domain:
  - Only [Backend Engineer](.config/claude/agents/backend-engineer.md) builds ETL pipelines and provisions infrastructure
  - Only [Backend Engineer](.config/claude/agents/backend-engineer.md) owns API documentation
