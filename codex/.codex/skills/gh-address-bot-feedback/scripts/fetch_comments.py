#!/usr/bin/env python3
"""
Fetch PR conversation comments + reviews + review threads (inline threads)
...
Only comments and reviews authored by `coderabbitai` or
`chatgpt-codex-connector` are returned.
...
ALLOWED_AUTHORS = {"coderabbitai", "chatgpt-codex-connector"}
...
def _author_login(node: dict[str, Any]) -> str | None:
    author = node.get("author") or {}
    return author.get("login")


def _filter_author_nodes(nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [node for node in nodes if _author_login(node) in ALLOWED_AUTHORS]
...
def _filter_threads(threads: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for thread in threads:
        ...
        allowed_comments = _filter_author_nodes(comments)
        if not allowed_comments:
            continue
        updated_thread = dict(thread)
        updated_comments = dict(thread.get("comments") or {})
        updated_comments["nodes"] = allowed_comments
        updated_thread["comments"] = updated_comments
        filtered.append(updated_thread)
    return filtered
...
def fetch_all(owner: str, repo: str, number: int) -> dict[str, Any]:
    ...
    return {
        ...
        "conversation_comments": _filter_author_nodes(conversation_comments),
        "reviews": _filter_author_nodes(reviews),
        "review_threads": _filter_threads(review_threads),
    }
